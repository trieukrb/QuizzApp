...
const Login = () => {
    ...
    return (<div style={{backgroundImage: `url(${auth_img})`}}
                 className="relative w-screen h-screen bg-cover bg-center flex justify-center items-center">
        <div
            className="flex flex-col items-center w-8/10 sm:w-1/2 md:w-1/2 lg:max-w-110 bg-white rounded-xl py-7 px-3 relative">
            <div className="z-20 hidden md:block absolute w-80 h-80 -top-40 -left-60 rotate-40 animate-float bg-cover bg-center"
                 style={{backgroundImage: `url(${rocket_img})`}}></div>
            <div className="z-20 hidden md:block absolute w-80 h-80 top-20 -right-55 -rotate-40 animate-float bg-cover bg-center"
                 style={{backgroundImage: `url(${astron_img})`}}></div>
            <h1 className="font-bold mb-5 text-4xl text-center bg-gradient-to-r from-violet-500 to-purple-500 bg-clip-text text-transparent">Tquizlet</h1>
            {
                loading ? (<Loading/>) :
                    (<>
                        <form action="" className="w-full">
                            <div className="px-3 sm:px-5">
                                ...
                                {error === 'Firebase: Error (auth/invalid-credential).' ?
                                    <p className="mb-3 text-red-500 text-sm">Wrong email or password</p> : ''}
                                <button
                        ...
                        <div className="w-full mt-5">
                            <div className="px-3 sm:px-5">
                                <button onClick={() => handlePopupSignIn(new GoogleAuthProvider())}
                                        className="flex items-center justify-center py-2 bg-white hover:bg-gray-200 hover:scale-104 cursor-pointer focus:ring-blue-500 focus:ring-offset-blue-200 text-gray-700 w-full transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 rounded-lg">
                                    <svg
                                        viewBox="0 0 24 24"
                                        height="25"
                                        width="25"
